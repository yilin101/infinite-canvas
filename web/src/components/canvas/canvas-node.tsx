...
import { CanvasNodeType, type CanvasInputMode, type CanvasNodeData, type Position } from "@/types/canvas";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            {!isGroup ? <ConnectionHandleDot side="left" visible={hovered || isSelected || isConnecting} touchMode={inputMode !== "mouse"} onMouseDown={(event) => onConnectStart(event, data.id, "target")} /> : null}
            {!isGroup ? <ConnectionHandleDot side="right" visible={data.type !== CanvasNodeType.Config && (hovered || isSelected || isConnecting)} touchMode={inputMode !== "mouse"} onMouseDown={(event) => onConnectStart(event, data.id, "source")} /> : null}
